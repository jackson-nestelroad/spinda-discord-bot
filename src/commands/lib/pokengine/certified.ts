...
export class CertifiedCommand implements Command {
    ...
    public async run(bot: DiscordBot, msg: Message, args: string[]) {
        ...
        const randomMon = mons[Math.floor(Math.random() * mons.length)];
        ...
        embed.setTitle(randomMon.text);
        embed.setURL(this.baseUrl + randomMon.dexPath);
        embed.setImage(this.baseUrl + randomMon.imagePath);
